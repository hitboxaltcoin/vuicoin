...
public:
    ...
    // Vuicoin Specific WalletInitInterface InitPrivateSendSettings
 ...
 * between vuicoind, and vuicoin-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    if (mode == HMM_BITCOIN_QT)
        strUsage += HelpMessageOpt("-windowtitle=<name>", _("Sets a window title which is appended to \"Vuicoin Core - \""));
    ...
    strUsage += HelpMessageOpt("-sporkaddr=<vuicoinaddress>", strprintf(_("Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.")));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/hitboxaltcoin/vuicoin>";
    const std::string URL_WEBSITE = "<https://vuicoin.org>";
...
/** Sanity checks
 *  Ensure that Vuicoin Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Vuicoin Core version %s\n", FormatFullVersion());
}
...
{
    // Make sure only a single Vuicoin Core process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if Vuicoin Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if Vuicoin Core is started while in a temporary directory.\n",
        ...
        do {
            try {
                ...
                if (!fDisableGovernance && !fTxIndex
                   && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/hitboxaltcoin/vuicoin/pull/1817 and https://github.com/hitboxaltcoin/vuicoin/pull/1743
    ...
    // ********************************************************* Step 10c: schedule Vuicoin-specific tasks
