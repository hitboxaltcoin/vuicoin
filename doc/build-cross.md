Cross-compiliation of Vuicoin Core
===============================

Vuicoin Core can be cross-compiled on Linux to all other supported host systems. This is done by changing
the `HOST` parameter when building the dependencies and then specifying another `--prefix` directory when building Vuicoin.
...
When building Vuicoin Core, use
...
It's not possible to build the Vuicoin Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the Vuicoin Core source code).
