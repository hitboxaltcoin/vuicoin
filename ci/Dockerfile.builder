...
# vuicoin_hash
RUN git clone https://github.com/hitboxaltcoin/vuicoin_hash
RUN cd vuicoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} vuicoin
RUN useradd -u ${USER_ID} -g vuicoin -s /bin/bash -m -d /vuicoin vuicoin
...
RUN mkdir /vuicoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /vuicoin-src && \
